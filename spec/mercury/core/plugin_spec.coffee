...
describe "Mercury.Plugin", ->
  ...
  describe ".register", ->
    ...
    it "instantiates and returns the plugin it instantiated", ->
      spyOn(Mercury, 'Plugin', -> plugin: '_plugin_')
      expect( Klass.register('foo', foo: 'bar') ).to.eql(plugin: '_plugin_')
  ...
  describe ".unregister", ->
    ...
    it "calls .get to find the plugin", ->
      spyOn(Klass, 'get', => @mock)
      Klass.unregister('foo')
      expect( Klass.get ).calledWith('foo')

    it "calls release on the plugin", ->
      spyOn(Klass, 'get', => @mock)
      Klass.unregister('foo')
  ...
  describe ".get", ->
    ...
    beforeEach ->
      @instance = Klass.register('foo', foo: 'bar')

    it "returns the registered plugin by name", ->
      expect( Klass.get('foo') ).to.eql(@instance)
  ...
  describe ".all", ->
    ...
    it "returns all the registered plugins", ->
      expect( Klass.all() ).to.eql(foo: @instance, name: subject)
  ...
  describe "#constructor", ->
    ...
    it "calls #delegateActions", ->
      spyOn(Klass::, 'delegateActions')
      subject = new Klass(name: 'foo', actions: {foo: 'bar'})
      expect( subject.delegateActions ).calledWith(foo: 'bar')
  ...
  describe "#registerButton", ->
    ...
    it "assigns @button", ->
      subject.registerButton('_foo_')
      expect( subject.button ).to.eq('_foo_')
  ...
  describe "#triggerAction", ->
    ...
    it "calls the handler with the correct args within the scope of itself", ->
      @mock.hasAction.returns(true)
      subject.triggerAction(1, 2, 'foo')
      expect( subject.actions.html ).calledWith('html', 1, 2, 'foo')
      expect( subject.actions.html ).calledOn(subject)
  ...
  describe "#config", ->
    ...
    it "falls back to Mercury.configuration", ->
      ...
      Mercury.configure('foo:bar:baz', '_foo_')
      expect( subject.config('foo:bar:baz') ).to.eq('_foo_')
  ...
  describe "#onButtonClick", ->
    ...
    it "calls #triggerAction", ->
      spyOn(subject, 'triggerAction')
      subject.onButtonClick()
      expect( subject.triggerAction ).called
