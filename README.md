...
## Callbacks and the Event Lifecycle
...
place to add or change functionality and to install various event handling callbacks affecting Mercury behavior.
...
several things once Mercury is loaded, and we expose as many ways to do this as possible.  Bind to the event
using jQuery, Prototype, or Mercury directly:
...
#### Mercury
    ...
    onload: function() {
        Mercury.on('ready', function() { Mercury.saveUrl = '/content'; });
    }

### The Saved Event

When Mercury successfully saves the content to the server after the Save button is pressed it will fire the
'saved' event. You can bind to this event as you do for the 'ready' event:
...
Other events generated by Mercury may be available to be bound to in the same manner as those above. Refer to
the source code to discover which ones are available and may be useful to your application.
