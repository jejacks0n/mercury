...
describe "Carmenta.Panel", ->

  template 'carmenta/panel.html'

  beforeEach ->
    Carmenta.displayRect = {top: 20, left: 20, width: 200, height: 200}
    $.fx.off = true
  ...
  afterEach ->
    ...
    $(document).unbind('carmenta:hide:panels')

  describe "#build", ->

    it "builds an element", ->
      @panel = new Carmenta.Panel('/evergreen/resources/panel.html', 'foo', {appendTo: '#test', title: 'foo panel'})
      html = $('<div>').html(@panel.element).html()
      expect(html).toEqual('<div class="carmenta-panel loading" style="display:none;"><h1>foo panel</h1><div class="carmenta-panel-pane"></div></div>')

    it "appends to any element", ->
      @panel = new Carmenta.Panel('/evergreen/resources/panel.html', 'foo', {appendTo: '#panel_container', title: 'foo panel'})
      expect($('#panel_container .carmenta-panel').length).toEqual(1)


  describe "observed events", ->

    beforeEach ->
      @panel = new Carmenta.Panel('/evergreen/resources/panel.html', 'foo', {appendTo: '#test', title: 'foo panel', for: $('#button')})

    describe "custom event: resize", ->

      it "calls position", ->
        spy = spyOn(Carmenta.Panel.prototype, 'position').andCallFake(=>)
    ...
    describe "custom event: hide:panels", ->
      ...
      it "doesn't hide if it's the same dialog", ->
        @panel.element.css({display: 'block'})
        Carmenta.trigger('hide:panels', @panel)
  ...
  describe "#show", ->
    ...
    it "hides other panels and dialogs", ->
      spyOn(Carmenta.Panel.prototype, 'position')
      spyOn(Carmenta.Panel.prototype, 'appear')
  ...
  describe "#resize", ->
    ...
    it "calls makeDraggable", ->
      spy = spyOn(Carmenta.Panel.prototype, 'makeDraggable').andCallFake(=>)
      @panel.resize()
      expect(spy.callCount).toEqual(1)
  ...
  describe "#loadContent", ->
    ...
    it "sets the element html to be the data passed to it", ->
      ...
      expect(@panel.element.html()).toEqual('<h1>foo panel</h1><div class="carmenta-panel-pane" style="visibility: hidden; ">hello world!</div>')
