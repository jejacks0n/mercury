...
describe "Carmenta.Dialog", ->

  template 'carmenta/dialog.html'
  ...
  describe "constructor", ->
    ...
    beforeEach ->
      @buildSpy = spyOn(Carmenta.Dialog.prototype, 'build').andCallFake(=>)
      @bindEventsSpy = spyOn(Carmenta.Dialog.prototype, 'bindEvents').andCallFake(=>)
      @preloadSpy = spyOn(Carmenta.Dialog.prototype, 'preload').andCallFake(=>)

    it "expects a url and name", ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo')
      expect(@dialog.url).toEqual('/evergreen/responses/blank.html')
      expect(@dialog.name).toEqual('foo')

    it "accepts options", ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo', {for: 'something', foo: 'bar'})
      expect(@dialog.options).toEqual({for: 'something', foo: 'bar'})

    it "calls build", ->
      @dialog = new Carmenta.Dialog()
  ...
  describe "#build", ->
    ...
    it "builds an element", ->
      ...
      expect(html).toEqual('<div class="carmenta-dialog carmenta-foo-dialog loading" style="display:none"></div>', {appendTo: '#test'})

    it "appends to any element", ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo', {appendTo: '#dialog_container'})
      expect($('#dialog_container .carmenta-dialog').length).toEqual(1)
  ...
  describe "#preload", ->
    ...
    beforeEach ->
      @loadSpy = spyOn(Carmenta.Dialog.prototype, 'load').andCallFake(=>)

    it "calls load if configured", ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo', {appendTo: '#test', preload: true})
      expect(@loadSpy.callCount).toEqual(1)

    it "doesn't call load if configured", ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo', {appendTo: '#test', preload: false})
  ...
  describe "#toggle", ->
    ...
    beforeEach ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo', {appendTo: '#test'})
  ...
  describe "#resize", ->
    ...
    it "calls show", ->
      spy = spyOn(Carmenta.Dialog.prototype, 'show').andCallFake(=>)
      @dialog.resize()
  ...
  describe "#show", ->
    ...
    describe "when already loaded", ->
      ...
      it "calls position", ->
        spy = spyOn(Carmenta.Dialog.prototype, 'position').andCallFake(=>)
        @dialog.show()
        expect(spy.callCount).toEqual(1)
        expect(spy.argsForCall[0]).toEqual([true])

      it "calls appear", ->
        spy = spyOn(Carmenta.Dialog.prototype, 'appear').andCallFake(=>)
        @dialog.show()
        expect(spy.callCount).toEqual(1)
  ...
  describe "#load", ->
    ...
    beforeEach ->
      @spyFunction = ->
      Carmenta.dialogHandlers.foo = ->
      @dialog = new Carmenta.Dialog('/evergreen/responses/blank.html', 'foo', {appendTo: '#test', for: $('#button')})
    ...
    describe "on success", ->
      ...
      beforeEach ->
        @loadContentSpy = spyOn(Carmenta.Dialog.prototype, 'loadContent').andCallFake(=>)
      ...
      it "calls a dialog handler if there's one", ->
        spy = spyOn(Carmenta.dialogHandlers, 'foo').andCallFake(=>)
        @dialog.load()
        expect(spy.callCount).toEqual(1)
    ...
    describe "on failure", ->
      ...
      it "hides", ->
        spy = spyOn(Carmenta.Dialog.prototype, 'hide').andCallFake(=>)
        @dialog.load()
        expect(spy.callCount).toEqual(1)
      ...
      it "alerts the user", ->
        ...
        expect(@alertSpy.argsForCall[0]).toEqual(['Carmenta was unable to load /evergreen/responses/blank.html for the foo dialog.'])
